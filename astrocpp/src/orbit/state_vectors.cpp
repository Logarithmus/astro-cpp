...
double StateVectors::delta_E_from_delta_M(double delta_M, double e_cos_E1,
					  double e_sin_E1) const {
  double delta_E = delta_M, delta_E0;
  auto f = [&e_cos_E1, &e_sin_E1, &delta_M](double delta_E) {
    return delta_E - e_cos_E1 * std::sin(delta_E) +
	   e_sin_E1 * (1.0 - std::cos(delta_E)) - delta_M;
  };
...
StateVectors StateVectors::propagated(double std_grav_param,
  ...
  if (a > 0) {		     // Elliptic motion
    // Some intermediate quantities
    const double root = std::sqrt(mu * a), inv_root = 1 / std::sqrt(mu * a),
		 e_cos_E1 = 1 - r1_mag / a,
