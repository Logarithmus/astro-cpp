...
struct ValidationData {
  ValidationData(const EllipticOrbit &orbit, double t1, double t2,
...
ValidationData::ValidationData(const EllipticOrbit &orbit, double t1, double t2,
			       const Vector3D &v1, const Vector3D &v2)
...
void test_case_size() {
  std::cout << sizeof(EllipticOrbit) << ' ' << sizeof(TestCase) << ' '
	    << sizeof(LambertProblem) << std::endl;
}
...
void lambert_problem(const int N) {
  ...
  std::uniform_real_distribution<double> a(0.0, 1.0e16), e(0.0, 1.0),
      i(0.0, math::PI_OVER_2), W(0.0, math::TAU), w(0.0, math::TAU);

  // Generate random test cases
  const double mu = 1.1723328e18, M0 = 0.0, t0 = 0.0;
  ...
  long long reserve_time =
      duration_cast<nanoseconds>(reserve_end - reserve_start).count();
  ...
  std::uniform_real_distribution<double> time1(0.0, 0.50001),
      time2(0.50002, 1.0);
  for (int j = 0; j < N; ++j) {
    EllipticOrbit orbit(mu, a(rand_gen), e(rand_gen), i(rand_gen), W(rand_gen),
			w(rand_gen), M0, t0);

    double t1 = time1(rand_gen) * orbit.period(),
	   t2 = time2(rand_gen) * orbit.period();
  ...
  std::cout << "Generation took " << gen_time << " ns" << std::endl
	    << static_cast<double>(gen_time) / N << " ns per problem"
	    << std::endl;
  ...
  for (auto test_case : test_cases) {
    lambert_opt.emplace_back(test_case.r1, test_case.r2, test_case.delta_t, mu,
			     false, 0);
  }
  auto lambert_end = high_resolution_clock::now();
  long long elapsed =
      duration_cast<nanoseconds>(lambert_end - lambert_start).count();
  std::cout << "Lambert new: " << elapsed << " ns" << std::endl
	    << static_cast<double>(elapsed) / N << " ns per lambert"
	    << std::endl;
  ...
  for (auto test_case: test_cases)
  {
      lambert_izzo.emplace_back(test_case.state1.r, test_case.state2.r,
  test_case.delta_t, mu, false, 0);
  }
  auto lambert_end = high_resolution_clock::now();
  std::cout << "Lambert old: " << duration_cast<microseconds>(lambert_end -
  lambert_start).count() << std::endl;
  ...
  */
  ...
  for (int j = 0; j < N; ++j) {
    double delta_v1_opt =
	       (validation_data[j].v1 - lambert_opt[j].velocity.back().first)
		   .mag(),
	   delta_v2_opt =
	       (validation_data[j].v2 - lambert_opt[j].velocity.back().second)
		   .mag(),
	   // delta_v1_izzo = (test_cases[j].state1.v -
	   // lambert_izzo[j].get_v1().back()).mag(), delta_v2_izzo =
	   // (test_cases[j].state2.v - lambert_izzo[j].get_v2().back()).mag(),
    ...
    //( (delta_v2_izzo / v2_kepler) > 1e-6)   )
    {
      ...
      std::cout << "t1 = " << validation_data[j].t1 << " ("
		<< validation_data[j].t1 / validation_data[j].orbit.period()
		<< " T)\n";
      std::cout << "t2 = " << validation_data[j].t2 << " ("
		<< validation_data[j].t2 / validation_data[j].orbit.period()
		<< " T)\n";

      std::cout << "\n    v1_kepler     = " << v1_kepler
		<< ";  v2_kepler      = " << v2_kepler
		<< "\n    v1_opt        = " << v1_opt << ";  v2_opt         = "
		<< v2_opt
		//<< "\n    v1_izzo       = " << v1_izzo       << ";  v2_izzo =
		//" << v2_izzo
		<< "\n    delta_v1_opt  = " << delta_v1_opt
		<< ";  delta_v2_opt   = "
		<< delta_v2_opt
		// << "\n    delta_v1_izzo = " << delta_v1_izzo << ";
		// delta_v2_izzo  = " << delta_v2_izzo
		<< std::endl;
    }
  }
  auto check_end = high_resolution_clock::now();
  long long check_time =
      duration_cast<nanoseconds>(check_end - check_start).count();
  std::cout << "\nCheck: " << check_time << " ns" << std::endl
	    << static_cast<double>(check_time) / N << " ns per check"
	    << std::endl;
