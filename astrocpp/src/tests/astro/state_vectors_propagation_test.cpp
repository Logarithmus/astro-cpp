...
struct ValidationData {
  ValidationData(const EllipticOrbit &orb, double t1, double t2,
		 const StateVectors &state2);
...
ValidationData::ValidationData(const EllipticOrbit &orb, double t1, double t2,
			       const StateVectors &state2)
...
void state_vectors_propagation(const int N) {
  ...
  std::uniform_real_distribution<double> a(0.0, 1.0e13), e(0.1, 0.3),
      i(0.0, math::PI), W(0.0, math::TAU), w(0.0, math::TAU);

  // Generate random test cases
  // const double mu = 1.1723328e18,
  const double mu = 1.32712440018e20, M0 = 0.0, t0 = 0.0;
  ...
  for (auto test_case : test_cases) {
    state2_propagated.emplace_back(
	test_case.state1.propagated(mu, test_case.delta_t));
  }
  auto kepler_end = high_resolution_clock::now();
  long long elapsed =
      duration_cast<nanoseconds>(kepler_end - kepler_start).count();
  std::cout << "Propagation: " << elapsed << " ns" << std::endl
	    << static_cast<double>(elapsed) / N << " ns per problem"
	    << std::endl;
  ...
  for (int j = 0; j < N; ++j) {
    const double delta_r2 =
	(validation_data[j].state2.r - state2_propagated[j].r).mag();
    //          delta_v2 = (validation_data[j].state2.v -
    //          state2_propagated[j].v).mag(),
    ...
    validation_data[j].t1 / validation_data[j].orbit.period() << " T)\n";
	std::cout << "t2 = " << validation_data[j].t2 << " (" <<
    validation_data[j].t2 / validation_data[j].orbit.period() << " T)\n";

	std::cout << "\n    r2_orbit = " << r2_orbit << ";  v2_orbit = " <<
    v2_orbit
		  << "\n    r2_prop  = " << r2_prop  << ";  v2_prop  = " <<
    v2_prop
		  << "\n    delta_r2 = " << delta_r2 << ";  delta_v2 = " <<
    delta_v2
		  << std::endl;
  ...
  const double
      delta_r2 =
	  (validation_data[j_max].state2.r - state2_propagated[j_max].r).mag(),
      delta_v2 =
	  (validation_data[j_max].state2.v - state2_propagated[j_max].v).mag(),
  ...
  std::cout << "t1 = " << validation_data[j_max].t1 << " ("
	    << validation_data[j_max].t1 / validation_data[j_max].orbit.period()
	    << " T)\n";
  std::cout << "t2 = " << validation_data[j_max].t2 << " ("
	    << validation_data[j_max].t2 / validation_data[j_max].orbit.period()
	    << " T)\n";
