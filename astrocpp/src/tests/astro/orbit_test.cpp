...
double kepler_solver_landis(double M, double e) {
  ...
  double sin_E = std::sin(E), cos_E = std::cos(E), f = E - e * sin_E - M,
	 Df = 1.0 - e * cos_E, DDf = e * sin_E, DDDf = e * cos_E,
	 Df_Df = Df * Df, f_f = f * f;
...
double kepler_solver_newton_improved_guess(double M, double e) {
  ...
  // Finally get our initial guess
  double E = ((2.0 * r * w) / (w * w + w * q + q_sqr) + M) / d, E0;
  int iters = 0;
  do {
    ++iters;
    E0 = E;
