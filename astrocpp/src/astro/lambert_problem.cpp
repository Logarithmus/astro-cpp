...
namespace astro_cpp {
// Computes the derivatives of T (non-dimensional time of flight) with respect
// to x
void LambertProblem::dT_over_dx(double x, double T, double &DT, double &DDT,
				double &DDDT) {
  // Semi-major axis normalized with respect to one corresponding to the min
  // energy ellipse
  double a_m_over_a = 1.0 - x * x, a_over_a_m = 1.0 / a_m_over_a,
	 inv_y = 1.0 / std::sqrt(1.0 - lambda2 * a_m_over_a),
	 inv_y2 = inv_y * inv_y, inv_y3 = inv_y2 * inv_y;
  DT = a_over_a_m * (3.0 * T * x - 2.0 + 2.0 * lambda3 * x * inv_y);
  DDT = a_over_a_m *
	(3.0 * T + 5.0 * x * DT + 2.0 * (1.0 - lambda2) * lambda3 * inv_y3);
  DDDT = a_over_a_m * (7.0 * x * DDT + 8.0 * DT -
		       6.0 * (1.0 - lambda2) * lambda5 * x * inv_y3 * inv_y2);
}

// When computing T(x) in the single revolution case, a loss of precision is
// encountered when x -> 1. So in this case we use Battin's expression which
// involves ordinary hypergeometric function
double LambertProblem::hypergeometric_function(double z, double eps) {
  double Sj = 1.0, Cj = 1.0, Cj1 = 0.0, Sj1 = 0.0, err = 1.0;
  int j = 0;
  while (err > eps) {
...
double LambertProblem::T_from_x_Lagrange(double x, int revs) {
  // a_over_a_m - semi-major axis normalized with respect to one corresponding
  // to the min energy ellipse
  double a_m_over_a = 1.0 - x * x, a_over_a_m = 1.0 / a_m_over_a;
  if (a_over_a_m > 0) {
    // ellipse
    ...
    return 0.5 * (a_over_a_m * std::sqrt(a_over_a_m) *
...
double LambertProblem::T_from_x(double x, int revs) {
  double battin = 0.01, lagrange = 0.2, dist = std::abs(x - 1.0);

  // |x - 1| > 0.2
  if (dist > lagrange) {
    // We use Lancaster's expression for T
    double a_m_over_a = 1.0 - x * x, abs_a_m_over_a = std::abs(a_m_over_a),
	   z = std::sqrt(1 - lambda2 * a_m_over_a);
    double y = std::sqrt(abs_a_m_over_a), g = x * z + lambda * a_m_over_a, d;
    if (a_m_over_a > 0) {
  ...
  double a_m_over_a = 1.0 - x * x, abs_a_m_over_a = std::abs(a_m_over_a),
	 z = std::sqrt(1 - lambda2 * a_m_over_a);
  double eta = z - lambda * x, S1 = (1.0 - lambda - x * eta) * 0.5,
	 Q = hypergeometric_function(S1, 1e-11);
  Q *= (4.0 / 3.0);
  return (eta * eta * eta * Q + 4.0 * lambda * eta) * 0.5 +
	 revs * math::PI / (abs_a_m_over_a * std::sqrt(abs_a_m_over_a));
}

double LambertProblem::x_from_T_Householder(double T, double x0, int revs,
					    double eps, int iter_max) {
  int iter_count = 0;
  double err = 1.0, x = 0.0, T_of_x0 = 0.0, delta_T = 0.0, DT = 0.0, DDT = 0.0,
	 DDDT = 0.0;
  while ((err > eps) && (iter_count < iter_max)) {
    ...
    delta_T = T_of_x0 - T;
    double DT2 = DT * DT;
    x = x0 - delta_T * (DT2 - delta_T * DDT * 0.5) /
		 (DT * (DT2 - delta_T * DDT) + DDDT * delta_T * delta_T / 6.0);
    err = std::abs(x - x0);
    x0 = x;
    ++iter_count;
  }
...
LambertProblem::LambertProblem(const Vector3D &r1, const Vector3D &r2,
			       double tof, double mu, bool is_retrograde,
			       int max_revs) {
  // 1 - Getting lambda and T
  const double c = (r2 - r1).mag(), // chord
      r1_mag = r1.mag(), r2_mag = r2.mag(), inv_r1_mag = 1.0 / r1_mag,
	       inv_r2_mag = 1.0 / r2_mag,
	       s = 0.5 * (c + r1_mag + r2_mag); // semi-perimeter
  const Vector3D r1_hat = inv_r1_mag * r1, r2_hat = inv_r2_mag * r2,
  ...
  if ((max_possible_revs <= max_revs) && (max_possible_revs > 0) && (T < T0)) {
    int it = 0;
    double err = 1.0, T_min = T0, x_old = 0.0, x_new = 0.0, DT, DDT, DDDT;
    while (1) {
  ...
  for (auto x : x_vector) {
    ...
    v1_t = v_t * inv_r1_mag;
    v2_t = v_t * inv_r2_mag;
    velocity.emplace_back(v1_r * r1_hat + v1_t * t1_hat,
			  v2_r * r2_hat + v2_t * t2_hat);
  }
}
} // namespace astro_cpp
